...
## 1.Чем отличается var от let и const?
   ...
   var- это старый способ объявлять переменные в JavaScript. Эта переменная видна во всей функции, даже если объявлена внутри блока if или for.
 let и const более новые и безопасные варианты.Они имеют блочную область видимости, то есть работают только внутри фигурных скобок, где были объявлены. Это делает код более безопасным и предсказуемым.
 let позволяет изменять значение переменной после объявления.
 const используется для переменных, значение которых нельзя переназначить. Это помогает избежать случайных изменений и делает код понятнее.
 
## 2. Что такое неявное преобразование типов?

Неявное преобразование типов- это когда JavaScript сам автоматически меняет тип значения, чтобы выполнить какую-то операцию.
Это происходит, когда в выражении используются разные типы данных, например строка и число. Язык старается привести их к одному типу, чтобы операция могла выполниться.

## 3.Как работает == и === ?
== сравнивает значения и может менять типы данных, чтобы их сравнить. === сравнивает и значение, и тип, ничего не меняя. Например 5 == "5" даст true , а  5 === "5" даст false. В коде чаще используют ===, потому что результат более предсказуем.
# Источники:
